...
```bash
uv run python ${CLAUDE_PLUGIN_ROOT}/skills/memory/memory-search.py index            # build/refresh embeddings (Ollama: nomic-embed-text)
uv run python ${CLAUDE_PLUGIN_ROOT}/skills/memory/memory-search.py search "QUERY" -k 5
```
...
## Structural manifest
```bash
uv run python ${CLAUDE_PLUGIN_ROOT}/skills/memory/memory-index.py        # writes memory/index.json
uv run python ${CLAUDE_PLUGIN_ROOT}/skills/memory/memory-index.py --check  # summary only
```
